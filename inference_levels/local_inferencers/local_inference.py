...
class BaseLocalInference(torch.nn.Module):
    ...
    def create_wave_vectors(self, a_vector, b_vector, energy_metrix):
        ...
        energized_a_vector = self.zero_dim_softmax(
            energy_metrix
        ).T  # we transform here because b is on dim 1 of the energy mat
    ...
    def create_m_vector(self, vector_hat, vector_wave):
        ...
        m_vector = torch.concat(
            (vector_hat, vector_wave, sub_vec, mul_vec), dim=0
    ...
    def forward(self, a_hat: torch.Tensor, b_hat: torch.Tensor):
        ...
        vector_wave_a, vector_wave_b = self.create_wave_vectors(
            a_hat, b_hat, self.energy_metrix
        )
