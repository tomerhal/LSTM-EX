...
class BaseLocalInference(torch.nn.Module):
    def __init__(self,embedding_size):
        ...
        self.zero_dim_softmax = torch.nn.Softmax(1)
        self.first_dim_softmax = torch.nn.Softmax(2)
    ...
    def create_wave_vectors(self,a_vector,b_vector,energy_metrix):
        ...
        energized_a_vector = self.zero_dim_softmax(energy_metrix).permute(0,2,1)   # we transform here because b is on dim 1 of the energy mat
    ...
    def create_m_vector(self,vector_hat,vector_wave):
        ...
        m_vector = torch.concat((vector_hat,vector_wave,sub_vec,mul_vec),dim=2) 
        return m_vector

    def forward(self, a_hat: torch.Tensor, b_hat: torch.Tensor):
        self.energy_metrix = torch.matmul(a_hat,b_hat.permute(0,2,1))
